...
import {
  ...
  updateMockJobPosting,
  deleteMockJobPosting,
} from "@/api/operations/mockinterview-api";
...
export default function EditMockInterviewPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await getMockJobPostingById(jobId, token);
        if (response?.success && response.data) {
          const data = response.data;
          setTitle(data.title || "");
          setDescription(data.description || "");

          const payload = data.jd_payload || {};
          setExperience(payload.experience || "");
          setRoleCategory(payload.role_category || "");
          setDifficultyLevel(payload.difficulty_level || "");
          setInterviewTools(payload.interview_tools || []);
          setLocation(payload.location || "Remote");
          setEmploymentType(payload.employment_type || "Full-time");
          setSkills(payload.skills || []);
          setRequirements(payload.requirements || []);
          setResponsibilities(payload.responsibilities || []);
        } else {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await updateMockJobPosting(
        jobId,
        title,
        description,
        jd_payload,
      ...
      if (response?.success) {
        toast.success(response.message || "Mock interview updated successfully!");
        router.push("/mockinterviews");
      } else {
        toast.error(response?.message || "Failed to update mock interview");
      }
    } catch (error) {
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      const response = await deleteMockJobPosting(jobId, token);
      if (response?.success) {
        toast.success("Mock interview deleted successfully!");
        router.push("/mockinterviews");
      } else {
        toast.error(response?.message || "Failed to delete mock interview");
      }
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-muted-foreground">Loading mock interview...</div>
      </div>
    );
  }
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-3xl font-bold">Edit Mock Interview</h1>
              </div>
              <p className="text-muted-foreground">
                Update the mock interview configuration
              </p>
            </div>
            <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>Delete Mock Interview</DialogTitle>
                  <DialogDescription>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {roleCategoryOptions.map((option) => (
                    <button
                      key={option.value}
                      type="button"
                      onClick={() => setRoleCategory(option.value)}
            ...
            <CardContent className="flex items-center justify-between p-6">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Review your changes and update the mock interview.
                </p>
              </div>
